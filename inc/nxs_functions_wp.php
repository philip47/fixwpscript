...
if (!function_exists('nxs_checkAddLogTable')){ function nxs_checkAddLogTable(){ global $wpdb; $charset_collate = 'DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci'; 
  ...
  $installed_ver = get_option( "nxs_log_db_table_version" ); if ($installed_ver=='1.6') return true;
  ...
  delete_option("nxs_log_db_table_version"); add_option("nxs_log_db_table_version", '1.6');  
...
if (!function_exists('nxs_checkAddQueryTable')){ function nxs_checkAddQueryTable() { global $wpdb; $charset_collate = ''; $table_name = $wpdb->prefix.'nxs_query'; 
    ...
    $installed_ver = get_option( "nxs_query_db_table_version" ); if ($installed_ver=='1.4') return true;    
    ...
    $sql = "CREATE TABLE $table_name (
        ...
        timetorun datetime DEFAULT CURRENT_TIMESTAMP NOT NULL,
    ...
    delete_option("nxs_query_db_table_version"); add_option("nxs_query_db_table_version", '1.4');
...
if (!function_exists('nxs_do_this_hourly')){ function nxs_do_this_hourly() { global $wpdb, $nxs_SNAP; // nxsLogIt('Hourly Event');
  ...
  if (isset($nxs_SNAP)) $options = $nxs_SNAP->nxs_options;  if (!empty($options) && !empty($options['numLogRows'])) $numLogRows = $options['numLogRows']; else $numLogRows = 1000;
	...
	$wpdb->query(
		$wpdb->prepare(
			"UPDATE {$table_name} SET flt = %s WHERE flt IS NULL OR flt = %s",
			'snap',
			''
		)
	);
...
// Delete rows where 'flt' is "cron" and 'id' is not in the last 360 records
	$wpdb->query(
		$wpdb->prepare(
			"DELETE FROM {$table_name} WHERE flt = %s AND id NOT IN (
            SELECT id FROM (
                SELECT id FROM {$table_name} ORDER BY id DESC LIMIT 360
            ) foo
        )",
			'cron'
		)
...
// Delete rows where 'id' is less than or equal to the 'id' at the offset specified by $numLogRows
  ...
  if (isset($options['errNotifEmailCB']) && (int)$options['errNotifEmailCB'] == 1 && isset($options['errNotifEmail']) && trim($options['errNotifEmail']) != '') { $logToSend = maybe_unserialize(get_option('NSX_LogToEmail')); //  prr($logToSend);
    if (is_array($logToSend) && count($logToSend)>0) { $to = sanitize_email($options['errNotifEmail']); $subject = "SNAP Error Log for ".sanitize_text_field($_SERVER["SERVER_NAME"]); $message = print_r($logToSend, true);
      $eml = get_bloginfo('admin_email'); if (trim($eml)=='') $eml = "snap-notify@".str_ireplace('www.','',sanitize_text_field($_SERVER["SERVER_NAME"])); 
      $headers = "From: " . sanitize_email($eml) . "\r\n"; $headers .= "Reply-To: ". sanitize_email($eml) . "\r\n"; $headers .= "MIME-Version: 1.0\r\n";
      ...
      if ($retval == true) $logMsg = array('type'=>'S', 'msg'=>'Log sent to email '.$options['errNotifEmail'], 'extInfo'=>count($logToSend).' records sent');  
        else  $logMsg = array('type'=>'ER', 'msg'=>'[FAILED] Log to email '.$options['errNotifEmail'], 'extInfo'=>count($logToSend).' records were NOT sent');          
